...
class SegmentTreeBeats {
  ...
  void push(int k) {
    ...
    if(max_v[k] < max_v[2*k+1]) {
      ...
      hmax_v[2*k+1] = max(hmax_v[2*k+1], max_v[k]);
    ...
    if(min_v[2*k+1] < min_v[k]) {
      ...
      if(max_v[2*k+1] == min_v[2*k+1]) {
        ...
        hmax_v[2*k+1] = max(hmax_v[2*k+1], min_v[k]);
    ...
    if(max_v[k] < max_v[2*k+2]) {
      ...
      hmax_v[2*k+2] = max(hmax_v[2*k+2], max_v[k]);
    ...
    if(min_v[2*k+2] < min_v[k]) {
      ...
      if(max_v[2*k+2] == min_v[2*k+2]) {
        ...
        hmax_v[2*k+2] = max(hmax_v[2*k+2], min_v[k]);
  ...
  SegmentTreeBeats(int n, ll *a) : n(n) {
    ...
    if(a != nullptr) {
      for(int i=0; i<n; ++i) {
        hmax_v[n0-1+i] = max_v[n0-1+i] = min_v[n0-1+i] = sum[n0-1+i] = a[i];
        smax_v[n0-1+i] = -inf;
        smin_v[n0-1+i] = inf;
      ...
      for(int i=n; i<n0; ++i) {
        hmax_v[n0-1+i] = max_v[n0-1+i] = min_v[n0-1+i] = sum[n0-1+i] = 0;
        smax_v[n0-1+i] = -inf;
        smin_v[n0-1+i] = inf;
        max_c[n0-1+i] = min_c[n0-1+i] = 1;
    ...
    for(int i=n; i<n0; ++i) {
      hmax_v[n0-1+i] = max_v[n0-1+i] = smax_v[n0-1+i] = 0;
  ...
  void debug() {
    for(int i=0; i<2*n0; ++i) {
      printf("%d: mx = %lld, smx = %lld, mc = %lld, hmx = %lld, mn = %lld, smn = %lld, mc = %lld "
          "sm = %lld, lad = %lld, lvl = %lld\n",
          i, max_v[i], smax_v[i], max_c[i], hmax_v[i], min_v[i], smin_v[i], min_c[i], sum[i], ladd[i], lval[i]);
    }
  }
};
