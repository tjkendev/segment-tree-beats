...
class SegmentTree {
  ...
  struct MinVal {
    ...
    void merge2(MinVal &l, MinVal &r) {
      nv_hmax = max(l.nv_hmax, r.nv_hmax);
      ...
      if(l.v < r.v) {
        ...
        v_hmax = r.v_hmax;
        nv_hmax = max(nv_hmax, l.v_hmax);
      } else {
        v = l.v;
        sv = min(l.sv, r.sv);
        c = l.c + r.c;

        v_hmax = max(l.v_hmax, r.v_hmax);
      }
    }
    void merge(MinVal &mv) {
      nv_hmax = max(nv_hmax, mv.nv_hmax);
      if(v < mv.v) {
        sv = min(sv, mv.v);

        nv_hmax = max(nv_hmax, mv.v_hmax);
      } else if(v > mv.v) {
        sv = min(v, mv.sv);
        v = mv.v;
        c = mv.c;

        nv_hmax = max(nv_hmax, v_hmax);
        v_hmax = mv.v_hmax;
      } else {
        sv = min(sv, mv.sv);
        c += mv.c;

        v_hmax = max(v_hmax, mv.v_hmax);
      }
    }
    void add(ll x) {
      if(v != inf) v += x;
  ...
  ll _query_hist_max_max(int a, int b, int k, int l, int r) {
    ...
    if(a <= l && r <= b) {
      return max(max(dmin[k].v_hmax, dmin[k].nv_hmax), max(dmin2[k].v_hmax, dmin2[k].nv_hmax));
    }
    push(k);
...
public:
  SegmentTree(int n, ll *a = nullptr) : n(n) {
    ...
    for(int i=0; i<n; ++i) {
      ...
      dmin[n0-1+i].v_hmax = a[i]; dmin[n0-1+i].nv_hmax = -inf;
      dmin2[n0-1+i].v_hmax = dmin2[n0-1+i].nv_hmax = -inf;
      dmin_s[n0-1+i] = 0;
    }
    for(int i=n; i<n0; ++i) {
      ...
      dmin2[n0-1+i].v = dmin2[n0-1+i].sv = inf; dmin2[n0-1+i].c = 0;
      dmin[n0-1+i].v_hmax = dmin[n0-1+i].nv_hmax = -inf;
      dmin2[n0-1+i].v_hmax = dmin2[n0-1+i].nv_hmax = -inf;
      dmin_s[n0-1+i] = 0;
    }
