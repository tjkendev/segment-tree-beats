...
class SegmentTree {
  ...
  ll min_hmax[4*N], nmin_hmax[4*N];
  ll min_hmin[4*N], nmin_hmin[4*N];

  void update_node_min(int k, ll x) {
    min_s[k] += (x - min_v[k]) * min_c[k];
    min_hmax[k] += (x - min_v[k]);
    min_hmin[k] += (x - min_v[k]);
  ...
  void addall(int k, ll a) {
    cur_s[k] += a * len[k];
    cur_ma[k] += a;
    // min_hmax[k] += a; nmin_hmax[k] += a;
    // min_hmin[k] += a; nmin_hmin[k] += a;
  ...
  void update(int k) {
    ...
    nmin_hmax[k] = max(nmin_hmax[2*k+1], nmin_hmax[2*k+2]);
    nmin_hmin[k] = min(nmin_hmin[2*k+1], nmin_hmin[2*k+2]);
    ...
    if(min_v[2*k+1] > min_v[2*k+2]) {
      ...
      min_hmax[k] = min_hmax[2*k+2];
      nmin_hmax[k] = max(nmin_hmax[k], min_hmax[2*k+1]);

      min_hmin[k] = min_hmin[2*k+2];
      nmin_hmin[k] = min(nmin_hmin[k], min_hmin[2*k+1]);
    } else if(min_v[2*k+1] < min_v[2*k+2]) {
      min_v[k] = min_v[2*k+1];
      min_c[k] = min_c[2*k+1];
      smin_v[k] = min(smin_v[2*k+1], min_v[2*k+2]);

      min_hmax[k] = min_hmax[2*k+1];
      nmin_hmax[k] = max(nmin_hmax[k], min_hmax[2*k+2]);

      min_hmin[k] = min_hmin[2*k+1];
      nmin_hmin[k] = min(nmin_hmin[k], min_hmin[2*k+2]);
    } else {
      min_v[k] = min_v[2*k+1];
      min_c[k] = min_c[2*k+1] + min_c[2*k+2];
      smin_v[k] = min(smin_v[2*k+1], smin_v[2*k+2]);

      min_hmax[k] = max(min_hmax[2*k+1], min_hmax[2*k+2]);

      min_hmin[k] = min(min_hmin[2*k+1], min_hmin[2*k+2]);
    }
  }
  ...
  ll _query_hist_max_max(int a, int b, int k, int l, int r) {
    ...
    if(a <= l && r <= b) {
      return max(min_hmax[k], nmin_hmax[k]);
    }
    push(k);
  ...
  ll _query_hist_max_min(int a, int b, int k, int l, int r) {
    if(b <= l || r <= a) {
      return inf;
    }
    if(a <= l && r <= b) {
      return min(min_hmin[k], nmin_hmin[k]);
    }
    push(k);
    ll lv = _query_hist_max_min(a, b, 2*k+1, l, (l+r)/2);
    ll rv = _query_hist_max_min(a, b, 2*k+2, (l+r)/2, r);
    return min(lv, rv);
  }
...
public:
  SegmentTree(int n, ll *a) : n(n) {
    ...
    for(int i=0; i<n; ++i) {
      cur_ma[n0-1+i] = cur_v[n0-1+i] = cur_s[n0-1+i] = a[i];
      min_v[n0-1+i] = 0;
      smin_v[n0-1+i] = inf;
      min_c[n0-1+i] = 1;

      min_hmax[n0-1+i] = min_hmin[n0-1+i] = a[i];
      nmin_hmax[n0-1+i] = -inf; nmin_hmin[n0-1+i] = inf;
    }
    for(int i=n; i<n0; ++i) {
      ...
      min_hmax[n0-1+i] = nmin_hmax[n0-1+i] = -inf;
      min_hmin[n0-1+i] = nmin_hmin[n0-1+i] = inf;
    }
  ...
  ll query_hmax_min(int a, int b) {
    return _query_hist_max_min(a, b, 0, 0, n0);
  }
  ...
  while(1) {
    ...
    switch(rtype(mt)) {
      ...
      case 3:
        ...
        break;
      case 4:
        r0 = stb.query_hmax_min(a, b);
        r1 = 1e18;
        for(int i=a; i<b; ++i) {
          r1 = min(r1, w[i]);
        }
        if(r0 != r1) {
          cout << "query hmax min (" << a << ", " << b << ") : " << r0 << " " << r1 << endl;
        }
        break;
      default:
        continue;
