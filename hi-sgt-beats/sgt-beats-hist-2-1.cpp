...
class SegmentTree {
  ...
  MinVal max_d[4*N], nval_d[4*N], min_d[4*N];
  ...
  void push(int k) {
    ...
    if(min_v[k] == min_v[2*k+1]) {
      ...
      min_d[2*k+1].update_min(max_d[k].min_v);
    } else {
      min_d[2*k+1].update_min(nval_d[k].min_v);
    }

    if(max_v[k] == max_v[2*k+1]) {
      max_d[2*k+1].update_min(max_d[k].min_v);
    } else if(min_v[k] == max_v[2*k+1]) {
      // max_v[2*k+1] == min_v[2*k+1] == min_v[k]
      max_d[2*k+1].update_min(min_d[k].min_v);
    } else {
      max_d[2*k+1].update_min(nval_d[k].min_v);
    }

    nval_d[2*k+1].update_min(nval_d[k].min_v);
    ...
    if(min_v[k] == min_v[2*k+2]) {
      ...
      min_d[2*k+2].update_min(max_d[k].min_v);
    } else {
      min_d[2*k+2].update_min(nval_d[k].min_v);
    }

    if(max_v[k] == max_v[2*k+2]) {
      max_d[2*k+2].update_min(max_d[k].min_v);
    } else if(min_v[k] == max_v[2*k+2]) {
      // max_v[2*k+2] == min_v[2*k+2] == min_v[k]
      max_d[2*k+2].update_min(min_d[k].min_v);
    } else {
      max_d[2*k+2].update_min(nval_d[k].min_v);
    }

    nval_d[2*k+2].update_min(nval_d[k].min_v);
  }
  ...
  ll _query_hist_max_sum(int a, int b, int k, int l, int r) {
    ...
    if(a <= l && r <= b) {
      ...
      if(max_v[k] == min_v[k]) {
        return sum[k] + max_d[k].sum;
      }
      return sum[k] + max_d[k].sum + nval_d[k].sum + min_d[k].sum;
    }
    push(k);
  ...
  void debug() {
    for(int k=0; k<2*n0-1; ++k) {
      ...
      printf("max = {%lld, %lld, %lld}, min = {%lld, %lld, %lld}, sum = %lld, Cmin = {%lld, %lld, %lld}, Cnmin = {%lld, %lld, %lld}, Cmmin = {%lld, %lld, %lld}, Csum = %lld, ladd = %lld\n",
          ...
          max_d[k].sum + nval_d[k].sum + min_d[k].sum, ladd[k]
          );
    }
  }
};
