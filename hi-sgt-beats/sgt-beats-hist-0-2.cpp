...
class SegmentTree {
  ...
  void push(int k) {
    ...
    if(s_ladd[k] != 0) {
      sum_addall(2*k+1, s_ladd[k]);
      sum_addall(2*k+2, s_ladd[k]);
      s_ladd[k] = 0;
    }
  ...
  ll _query_cum_sum(int a, int b, int k, int l, int r) {
    if(b <= l || r <= a) {
      return 0;
    }
    if(a <= l && r <= b) {
    ...
    push(k);
    ll lv = _query_cum_sum(a, b, 2*k+1, l, (l+r)/2);
    ll rv = _query_cum_sum(a, b, 2*k+2, (l+r)/2, r);
    return lv + rv;
  }
...
public:
  SegmentTree(int n, ll *a = nullptr) : n(n) {
    ...
    for(int i=0; i<n; ++i) {
      sum_a[n0-1+i] = cur_ma[n0-1+i] = cur_mi[n0-1+i] = cur_v[n0-1+i] = cur_s[n0-1+i] = (a != nullptr ? a[i] : 0);
      hmax[n0-1+i].init();
      hmin[n0-1+i].init();
    }
    for(int i=n; i<n0; ++i) {
  ...
  ll query_hmax_sum(int a, int b) {
    return _query_sum(a, b, 0, 0, n0) + _query_hist_max_sum(a, b, 0, 0, n0);
  }

  ll query_hmin_sum(int a, int b) {
    return _query_sum(a, b, 0, 0, n0) - _query_hist_min_sum(a, b, 0, 0, n0);
  }

  ll query_cum_sum(int a, int b, ll t) {
    return _query_sum(a, b, 0, 0, n0) * t + _query_cum_sum(a, b, 0, 0, n0);
  }
...
int main() {
  ...
  for(int i=0; i<n; ++i) s[i] = u[i] = v[i] = w[i] = val(mt);
  ...
  while(1) {
    ...
    switch(rtype(mt)) {
      ...
      case 4:
        ...
        break;
      case 5:
        r0 = stb.query_cum_sum(a, b, t);
        r1 = 0;
        for(int i=a; i<b; ++i) {
          r1 += s[i];
        }
        if(r0 != r1) {
          cout << "query cumulative sum (" << a << ", " << b << ") (t = " << t << ") : " << r0 << " " << r1 << endl;
        }
        break;
      default:
        continue;
